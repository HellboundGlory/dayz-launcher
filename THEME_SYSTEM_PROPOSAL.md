...
**Status: PROPOSAL — no implementation yet.** This document is the deliverable
requested for a fully portable, exportable/importable UI customization system.
It is grounded in a direct audit of this repository (file:line citations
throughout) plus research into comparable systems (Playnite, VS Code, Obsidian,
BetterDiscord/Vencord, Home Assistant Lovelace, Shopify theme sections). Stop
point: review this document before any code changes begin.
...
| Slot ID | Required children | Optional children | `themeable` |
|---|---|---|---|
| `shell.sidebar` | `navList`, `settingsEntry` | `collapseToggle`, `logo` | `full` |
| `shell.header` | `windowControls` | `dragRegion` | `full` |
| `shell.footer` | `steamStateChip` | `serverCounts`, `uiScaleSlider` | `full` |
| `filterBar` | `searchInput`, `refreshAction` | `mapFilter`, `tagsFilter`, `sortControl`, `pingSlider` | `full` |
| `server.row` | `name`, `joinAction`, `modStatusBadge` | `pingBadge`, `playerCount`, `tagsLine`, `regionFlag` | `full` |
| `server.rowActions` | `joinAction` | `moreInfoItem`, `loadToMenuItem`, `downloadModsItem` | `full` |
| `modal.serverInfo` | `closeAction`, `joinAction` | `statGrid`, `readinessStrip`, `propsList` | `full` |
| `modal.modFilter` | `closeAction`, `applyAction` | `tabStrip`, `previewPane` | `full` |
| `mods.toolbar` | `searchInput`, `refreshAction` | `statusFilter` | `full` |
| `mods.row` | `modName`, `modStatusBadge` | `sizeLabel`, `usageCount` | `full` |
| `mods.inspector` | `closeAction` | `detailFields` | `full` |
| `modal.onboarding` | `primaryAction` | `pathBrowser` | `full` |
| `modal.update` | `closeAction`, `installAction` | `changelogView` | `full` |
| `modal.steamRequired` | `retryAction` | `errorCopy` | `full` |
| `settings.*` | *(all)* | *(none)* | `full` |
...
For the two highest-value slots (`server.row`, `mods.row`), Expert themes may
supply a composition file (`components/server-row.json`) built from a closed
...
- `type: "core"` is the **only** leaf type, and `ref` must resolve against
  `slots.ts`'s child list for that slot — there is no `type: "html"`,
  `type: "iframe"`, or `type: "script"`, and there never will be under this
  design. `type: "stack" | "box" | "grid"` are pure layout containers with a
  fixed prop set (`direction`, `gap`, `align`, `justify`, `wrap`).
- The auto-append rule from §3.3 applies identically here: any `required`
  child not reachable by walking this tree is appended after it.
...
Expert-tier-only and absent from Basic/Advanced packages.
...
### 9.1 Is there a visual/WYSIWYG editor? Basic tier now, Advanced/Expert in Phase 3.5 — no separate bundled app
...
For Advanced/Expert tier (`layout.json`, `styles.css`,
`components/*.json`), Phases 0–3 ship text-file authoring only (whatever
editor the theme author already uses, plus live hot-reload, the Developer
Mode slot inspector, and inline validation) — **Phase 3.5 then adds an
in-launcher visual layout builder on top of that same Developer Mode
surface** (§15): drag handles to reorder a slot's children, a right-click
"hide this element" action, and size handles on resizable slots, all
writing `layout.json`/`components/*.json` for the author instead of asking
them to hand-edit it. Text editing keeps working throughout — the visual
builder reads and writes the same files, it doesn't replace them with a
separate project format.

**No separate bundled editor application is proposed**, deliberately. A
standalone "Theme Studio" exe would have to re-implement or mirror the real
launcher UI to preview anything accurately — a second rendering surface to
keep in sync with every future core UI change, which directly fights the
"themes must survive launcher updates automatically" goal (§3.3) instead of
serving it. The running launcher is already the correct preview surface: it
*is* the renderer the theme will actually apply to, live-reloading a saved
file in ~100ms is strictly faster than any external preview pane could be,
and it needs no second codebase to maintain — the visual builder in Phase
3.5 is a mode of the real app, not a second app.

This is feasible to build with confidence, not a moonshot, specifically
*because* `layout.json`/`components/*.json` are closed-vocabulary declarative
data (§4), not arbitrary code — a GUI can read and write them directly
without interpreting anything unsafe. Every write the visual builder makes
is itself just another theme activation, so it goes through the same
Theme Activation Safety Window as any other change (§7.4) — dragging a
row into an unreadable arrangement is exactly the case that mechanism
exists for.
...
-------------------------------------------------------------------
 ...
 v  Theme      Colors, installed themes, layout, import/export
     ...
     [Customize tokens] [Edit layout] [Theme settings] [Developer Mode: Off]
     [Reset to Default]
```
...
14. `components/*.json` (if present) only uses the closed primitive
    vocabulary and only `type: "core"` refs known to `slots.ts` (§4.3).
...
- Assets are referenced from `tokens.json`/`layout.json`/`styles.css` only
  by path relative to the theme's own root (`assets/backgrounds/bg1.webp`) —
  never an absolute path, never a path outside the package.
...
3. **"Tactical HUD" — Expert tier.** Sidebar repositioned to a bottom
   ...
   compact stack ahead of `name`) while `joinAction` and
   `modStatusBadge` remain exactly where the required-child rule (§3.3)
   places any composition that doesn't explicitly position them.
...
| `src/components/themes-page/` (Phase 3.5) | Developer Mode gains drag-to-reorder/visibility-toggle/resize-handle controls that write `layout.json`/`components/*.json` directly (§9.1) |
...
**Phase 3.5 — In-launcher visual layout builder.**
Drag-to-reorder, visibility-toggle, and resize-handle controls added to
Developer Mode (§9.1), writing `layout.json`/`components/*.json` directly
instead of requiring hand-edits; every change it makes still flows through
the Phase 1 Theme Activation Safety Window like any other activation.
